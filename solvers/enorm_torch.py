...
with safe_import_context() as import_ctx:
    ...
    class ENorm:
        ...
        Args:
            ...
            - optimizer: the optimizer, necessary for the momentum buffer
            update
            Note: only torch.optim.SGD supported for the moment
            - model_type: choose among ['linear', 'conv'] (see main.py)
            - c: asymmetric scaling factor that introduces a depth-wise
            penalty on the weights (default:1)
        ...
        Notes:
            ...
            - In practice, we have found the training to be more stable when
            we do not balance the biases
        """

        def filter_map(self, x):
            return not any(name in x[0] for name in self.to_remove)
        ...
        def __init__(self, named_params, optimizer, model_type, c=1, p=2):
            ...
            # weights and biases
            self.weights = [
                (n, p) for n, p in self.named_params if "weight" in n
            ]
            self.weights = list(filter(self.filter_map, self.weights))
            self.biases = []
        ...
        def _step_fc(self):
            ...
            for i in range(1, self.n_layers - 1):
                balancer = (right_norms / (left_norms * self.C[i - 1])).pow(
                    self.alpha
                )

                left_norms = self._get_weight(i).norm(p=self.p, dim=1).data
                right_norms = (
                    self._get_weight(i + 1).norm(p=self.p, dim=0).data
                )

                if len(self.biases) > 0:
                    self.biases[i - 1][1].data.mul_(balancer)
        ...
        def _step_conv(self):
            ...
            for i in range(1, self.n_layers - 1):
                ...
                if self.momentum:
                    self.optimizer.state[self.weights[i - 1][1]][
                        "momentum_buffer"
                    ].mul_(1 / balancer.unsqueeze(1).unsqueeze(2).unsqueeze(3))
                    self.optimizer.state[self.weights[i][1]][
                        "momentum_buffer"
                    ].mul_(balancer.unsqueeze(1).unsqueeze(2).unsqueeze(0))
